...
CameraNode::CameraNode(const rclcpp::NodeOptions & options) : Node("camera_node", options)
{
    ...
    this->declare_parameter("camera_num", 4);
    this->declare_parameter("publish_freq", 100);
    this->get_parameter("camera_num", camera_num);
    this->get_parameter("publish_freq", publish_freq);

    RCLCPP_INFO( this->get_logger(), "\nCamera Node Started.");
    image_pub_ = this->create_publisher<sensor_msgs::msg::Image>("camera_img", 10);
    timer_ = this->create_wall_timer(std::chrono::milliseconds(publish_freq), std::bind(&CameraNode::callback, this));
