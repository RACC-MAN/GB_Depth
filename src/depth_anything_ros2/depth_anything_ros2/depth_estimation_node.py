...
class DepthEstimationNode(Node):
    ...
    def __init__(self):
        ...
        self.image_sub_ = self.create_subscription(Image, '/camera/camera/color/image_raw', self.img_callback, 10)
        # self.image_sub_ = self.create_subscription(Image, '/camera_img', self.callback, 10)
    ...
    def timer_callback(self):
        if self.img_ptr_ == None:
            return
        raw_img = self.bridge.imgmsg_to_cv2(self.img_ptr_, 'bgr8')
        # self.get_logger().info(f"h: {raw_img.shape[0]}, w: {raw_img.shape[1]}")
        ...
        # ---- ベクトル化計算 ----
        value = (2 * maxDe - depth) / rangeDe   # (H, W) の行列演算

        # ピクセル座標差をベクトルで生成
        y_indices = np.arange(height).reshape(-1, 1) - ch
        pix_dist_inv = 1.0 / self.SENSOR_DIST_

        # 高さマップを一括計算
        h_val = value * y_indices * pix_dist_inv

        # flattenしてlist化
        depth_n_data.data = value.flatten().tolist()
        depth_data.data   = value.flatten().tolist()
        height_data.data  = h_val.flatten().tolist()

        max_h, min_h = np.max(h_val), np.min(h_val)
        ...
        depth_image = self.bridge.cv2_to_imgmsg(depth, "mono8", self.img_ptr_.header)
